...
// @description  Generates HTML, JSON, TXT output of your Steam Workshop subscriptions for a game.
// @version      1.0.3
// @author       spacegears
...
(function() {
	...
	function createFileLink (contEle, saveType, allData) {
		...
		newEle.id = eleId;
		newEle.classList.add(scriptName+'-links');
	...
	function dataToHTML (data) {
		...
		let output = [
            ...
            '     - Created: '+created+'',
			'     - Steam app: '+appName+' (App ID: '+appID+')',
			'     - Steam user: '+appUser+' (Profile ID: '+profileID+')',
		...
		data.map(item => {
            output.push('<div class="item"><a href="'+item.link+'" target="_blank"><img src="'+item.thumbnail+'" title="'+item.name+' (Last updated: '+item.updated+')"></a>'+item.name+'</div>');
		});
	...
	function runSWSL () {
        ...
        if (!appIDset || !swslRunning) {
            ...
            logMsg(`%c SWSL %cv${scriptVersion} `, "background: #000000; color: #046eb2", "background: #000000; color: #ffffff");
	...
	function createLinks () {
		...
		contEle.classList.add(scriptName+'-container');
		contEle.innerHTML = '';

		const headerEleId = scriptName+'-header';
		let headerEle = document.createElement('span');
		headerEle.id = contEleId;
		headerEle.innerHTML = 'Export list for:<br><strong>'+appName+'</strong>';
		contEle.appendChild(headerEle);
        headerEle.classList.add(scriptName+'-header');
		contEle.appendChild(headerEle);
